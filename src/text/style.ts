...
class PPTXTextStyleUtils {
    ...
    static slideFactor = PPTXConstants.SLIDE_FACTOR;
     ...
     */
    static getFontBold(node, type, slideMasterTextStyles) {
 ...
 */
static getFontItalic(node, type, slideMasterTextStyles) {
 ...
 */
static getTextVerticalAlign(node, type, slideMasterTextStyles) {
    var baseline: any = PPTXUtils.getTextByPathList(node, ["a:rPr", "attrs", "baseline"]);
 ...
 */
static getFontType(node, type, warpObj, pFontStyle) {
    var typeface: any = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:latin", "attrs", "typeface"]);

    if (typeface === undefined) {
        var fontIdx: any = "";
        var fontGrup: any = "";
        if (pFontStyle !== undefined) {
            fontIdx = PPTXUtils.getTextByPathList(pFontStyle, ["attrs", "idx"]);
        }
        var fontSchemeNode: any = PPTXUtils.getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:fontScheme"]);
        if (fontIdx == "") {
        ...
        typeface = PPTXUtils.getTextByPathList(fontSchemeNode, [fontGrup, "a:latin", "attrs", "typeface"]);
    }
 ...
 */
static getTextHorizontalAlign(node, pNode, type, warpObj) {
    var getAlgn: any = PPTXUtils.getTextByPathList(node, ["a:pPr", "attrs", "algn"]);
    if (getAlgn === undefined) {
        getAlgn = PPTXUtils.getTextByPathList(pNode, ["a:pPr", "attrs", "algn"]);
    }
    if (getAlgn === undefined) {
        if (type == "title" || type == "ctrTitle" || type == "subTitle") {
            var lvlIdx: any = 1;
            var lvlNode: any = PPTXUtils.getTextByPathList(pNode, ["a:pPr", "attrs", "lvl"]);
            if (lvlNode !== undefined) {
                lvlIdx = parseInt(lvlNode) + 1;
            }
            var lvlStr: any = "a:lvl" + lvlIdx + "pPr";
            getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideLayoutTables", "typeTable", type, "p:txBody", "a:lstStyle", lvlStr, "attrs", "algn"]);
            if (getAlgn === undefined) {
                getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTables", "typeTable", type, "p:txBody", "a:lstStyle", lvlStr, "attrs", "algn"]);
                if (getAlgn === undefined) {
                    getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:titleStyle", lvlStr, "attrs", "algn"]);
                    if (getAlgn === undefined && type === "subTitle") {
                        getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:bodyStyle", lvlStr, "attrs", "algn"]);
                    }
                }
            }
        } else if (type == "body") {
            getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:bodyStyle", "a:lvl1pPr", "attrs", "algn"]);
        } else {
            getAlgn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTables", "typeTable", type, "p:txBody", "a:lstStyle", "a:lvl1pPr", "attrs", "algn"]);
        }

    }

    var align: any = "inherit";
    if (getAlgn !== undefined) {
        switch (getAlgn) {
            case "l":
                align = "left";
 ...
 */
static getFontSize(node, textBodyNode, pFontStyle, lvl, type, warpObj) {
    // if(type == "sldNum")
    //console.log("getFontSize node:", node, "lstStyle", lstStyle, "lvl:", lvl, 'type:', type, "warpObj:", warpObj)
    var lstStyle: any = (textBodyNode !== undefined)? textBodyNode["a:lstStyle"] : undefined;
    var lvlpPr: any = "a:lvl" + lvl + "pPr";
    ...
    if (isNaN(fontSize) || fontSize === undefined && node["a:fld"] !== undefined) {
        sz = PPTXUtils.getTextByPathList(node["a:fld"], ["a:rPr", "attrs", "sz"]);
        fontSize = parseInt(sz) / 100;
    }
    if ((isNaN(fontSize) || fontSize === undefined) && node["a:t"] === undefined) {
        sz = PPTXUtils.getTextByPathList(node["a:endParaRPr"], [ "attrs", "sz"]);
        fontSize = parseInt(sz) / 100;
    }
    if ((isNaN(fontSize) || fontSize === undefined) && lstStyle !== undefined) {
        sz = PPTXUtils.getTextByPathList(lstStyle, [lvlpPr, "a:defRPr", "attrs", "sz"]);
        fontSize = parseInt(sz) / 100;
    }
    //a:spAutoFit
    var isAutoFit: any = false;
    var isKerning: any = false;
    var kern: any = undefined;
    if (textBodyNode !== undefined){
        var spAutoFitNode: any = PPTXUtils.getTextByPathList(textBodyNode, ["a:bodyPr", "a:spAutoFit"]);
        // if (spAutoFitNode === undefined) {
        //     spAutoFitNode = PPTXUtils.getTextByPathList(textBodyNode, ["a:bodyPr", "a:normAutofit"]);
        // }
        if (spAutoFitNode !== undefined){
            isAutoFit = true;
            isKerning = true;
    ...
    if (isNaN(fontSize) || fontSize === undefined) {
        ...
        //     lvlpPr = "a:lvl1pPr";
        // }
        sz = PPTXUtils.getTextByPathList(warpObj["slideLayoutTables"], ["typeTable", type, "p:txBody", "a:lstStyle", lvlpPr, "a:defRPr", "attrs", "sz"]);
        fontSize = parseInt(sz) / 100;
        kern = PPTXUtils.getTextByPathList(warpObj["slideLayoutTables"], ["typeTable", type, "p:txBody", "a:lstStyle", lvlpPr, "a:defRPr", "attrs", "kern"]);
    ...
    if (isNaN(fontSize) || fontSize === undefined) {
        ...
        //     lvlpPr = "a:lvl1pPr";
        // }
        sz = PPTXUtils.getTextByPathList(warpObj["slideMasterTables"], ["typeTable", type, "p:txBody", "a:lstStyle", lvlpPr, "a:defRPr", "attrs", "sz"]);
        kern = PPTXUtils.getTextByPathList(warpObj["slideMasterTables"], ["typeTable", type, "p:txBody", "a:lstStyle", lvlpPr, "a:defRPr", "attrs", "kern"]);
        if (sz === undefined) {
            if (type == "title" || type == "subTitle" || type == "ctrTitle") {
                sz = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:titleStyle", lvlpPr, "a:defRPr", "attrs", "sz"]);
                kern = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:titleStyle", lvlpPr, "a:defRPr", "attrs", "kern"]);
            } else if (type == "body" || type == "obj" || type == "dt" || type == "sldNum" || type === "textBox") {
                sz = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:bodyStyle", lvlpPr, "a:defRPr", "attrs", "sz"]);
                kern = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:bodyStyle", lvlpPr, "a:defRPr", "attrs", "kern"]);
            }
            else if (type == "shape") {
                //textBox and shape text does not indent
                sz = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:otherStyle", lvlpPr, "a:defRPr", "attrs", "sz"]);
                kern = PPTXUtils.getTextByPathList(warpObj["slideMasterTextStyles"], ["p:otherStyle", lvlpPr, "a:defRPr", "attrs", "kern"]);
                isKerning = false;
            }

            if (sz === undefined) {
                sz = PPTXUtils.getTextByPathList(warpObj["defaultTextStyle"], [lvlpPr, "a:defRPr", "attrs", "sz"]);
                kern = (kern === undefined)? PPTXUtils.getTextByPathList(warpObj["defaultTextStyle"], [lvlpPr, "a:defRPr", "attrs", "kern"]) : undefined;
                isKerning = false;
            }
    ...
    if (baseline !== undefined && !isNaN(fontSize)) {
        var baselineVl: any = parseInt(baseline) / 100000;
        //fontSize -= 10; 
    ...
    if (!isNaN(fontSize)){
        var normAutofit: any = PPTXUtils.getTextByPathList(textBodyNode, ["a:bodyPr", "a:normAutofit", "attrs", "fontScale"]);
 ...
 */
    static getFontColorPr(node, pNode, lstStyle, pFontStyle, lvl, idx, type, warpObj) {
    ...
    if (rPrNode !== undefined) {
        ...
        if (filTyp == "SOLID_FILL") {
            var solidFillNode: any = rPrNode["a:solidFill"];// PPTXUtils.getTextByPathList(node, ["a:rPr", "a:solidFill"]);
            color = PPTXColorUtils.getSolidFill(solidFillNode, undefined, undefined, warpObj);
            var highlightNode: any = rPrNode["a:highlight"];
            ...
            var pattFill: any = rPrNode["a:pattFill"];// PPTXUtils.getTextByPathList(node, ["a:rPr", "a:pattFill"]);
            color = PPTXColorUtils.getPatternFill(pattFill, warpObj);
            colorType = "pattern";
        } else if (filTyp == "PIC_FILL") {
            var picFillResult: any = PPTXColorUtils.getPicFill("slideBg", rPrNode["a:blipFill"], warpObj);
            ...
            var shpFill: any = rPrNode["a:gradFill"];
            color = PPTXColorUtils.getGradientFill(shpFill, warpObj);
            colorType = "gradient";
        } 
    }
    if (color === undefined && PPTXUtils.getTextByPathList(lstStyle, ["a:lvl" + lvl + "pPr", "a:defRPr"]) !== undefined) {
        //lstStyle
        var lstStyledefRPr: any = PPTXUtils.getTextByPathList(lstStyle, ["a:lvl" + lvl + "pPr", "a:defRPr"]);
...
shpFill = lstStyledefRPr["a:gradFill"];
    ...
    if (color === undefined) {
        var sPstyle: any = PPTXUtils.getTextByPathList(pNode, ["p:style", "a:fontRef"]);
        if (sPstyle !== undefined) {
            color = PPTXColorUtils.getSolidFill(sPstyle, undefined, undefined, warpObj);
            if (color !== undefined) {
                colorType = "solid";
            }
highlightNode = sPstyle["a:highlight"]; //is "a:highlight" node in 'a:fontRef' ?
        ...
        if (color === undefined) {
            if (pFontStyle !== undefined) {
                color = PPTXColorUtils.getSolidFill(pFontStyle, undefined, undefined, warpObj);
                if (color !== undefined) {
                    colorType = "solid";
                }
            }
    ...
    if (color === undefined) {
        ...
        var layoutMasterNode: any = PPTXLayoutUtils.getLayoutAndMasterNode(pNode, idx, type, warpObj);
        ...
        if (pPrNodeLaout !== undefined) {
            var defRpRLaout: any = PPTXUtils.getTextByPathList(pPrNodeLaout, ["a:defRPr", "a:solidFill"]);
...
highlightNode = PPTXUtils.getTextByPathList(pPrNodeLaout, ["a:defRPr", "a:highlight"]);
        ...
        if (color === undefined) {
            ...
            if (pPrNodeMaster !== undefined) {
                var defRprMaster: any = PPTXUtils.getTextByPathList(pPrNodeMaster, ["a:defRPr", "a:solidFill"]);
...
highlightNode = PPTXUtils.getTextByPathList(pPrNodeMaster, ["a:defRPr", "a:highlight"]);
    ...
    var txtEffObj: any = {}
    //textBordr
    var txtBrdrNode: any = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:ln"]);
    var textBordr: any = "";
    if (txtBrdrNode !== undefined && txtBrdrNode["a:noFill"] === undefined) {
        var txBrd: any = PPTXStyleManager.getBorder(node, pNode, false, "text", warpObj);
        var txBrdAry: any = txBrd.split(" ");
        //var brdSize = (parseInt(txBrdAry[0].substring(0, txBrdAry[0].indexOf("pt")))) + "px";
        var brdSize: any = (parseInt(txBrdAry[0].substring(0, txBrdAry[0].indexOf("px")))) + "px";
        var brdClr: any = txBrdAry[2];
    ...
    var txtGlowNode: any = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:effectLst", "a:glow"]);
    var oGlowStr: any = "";
    if (txtGlowNode !== undefined) {
        var glowClr: any = PPTXColorUtils.getSolidFill(txtGlowNode, undefined, undefined, warpObj);
        var rad: any = (txtGlowNode["attrs"]["rad"]) ? (txtGlowNode["attrs"]["rad"] * this.slideFactor) : 0;
    ...
    var txtShadow: any = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:effectLst", "a:outerShdw"]);
    var oShadowStr: any = "";
    if (txtShadow !== undefined) {
        ...
        var shadowClr: any = PPTXColorUtils.getSolidFill(txtShadow, undefined, undefined, warpObj);
        var outerShdwAttrs: any = txtShadow["attrs"];
        // algn: "bl"
        // dir: "2640000"
        // dist: "38100"
        ...
        var algn: any = outerShdwAttrs["algn"];
        var dir: any = (outerShdwAttrs["dir"]) ? (parseInt(outerShdwAttrs["dir"]) / 60000) : 0;
        var dist: any = parseInt(outerShdwAttrs["dist"]) * this.slideFactor;//(px) //* (3 / 4); //(pt)
        var rotWithShape: any = outerShdwAttrs["rotWithShape"];
        var blurRad: any = (outerShdwAttrs["blurRad"]) ? (parseInt(outerShdwAttrs["blurRad"]) * this.slideFactor + "px") : "";
        var sx: any = (outerShdwAttrs["sx"]) ? (parseInt(outerShdwAttrs["sx"]) / 100000) : 1;
        var sy: any = (outerShdwAttrs["sy"]) ? (parseInt(outerShdwAttrs["sy"]) / 100000) : 1;
        var vx: any = dist * Math.sin(dir * Math.PI / 180);
        var hx: any = dist * Math.cos(dir * Math.PI / 180);
    ...
    //     color = "" + color;
    // }
    var text_effcts: any = "", txt_effects;
    if (colorType == "solid") {
 ...
 */
    static getContentDir(node, type, warpObj) {
    var defRtl: any = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:lstStyle", "a:defPPr", "attrs", "rtl"]);
    if (defRtl !== undefined) {
        if (defRtl == "1"){
            return "content-rtl";
        } else if (defRtl == "0") {
    ...
    //var lvl1Rtl = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:lstStyle", "lvl1pPr", "attrs", "rtl"]);
    ...
    //     }
    // }
    var rtlCol: any = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:bodyPr", "attrs", "rtlCol"]);
    if (rtlCol !== undefined) {
        if (rtlCol == "1") {
            return "content-rtl";
        } else if (rtlCol == "0") {
    ...
    var slideMasterTextStyles: any = warpObj["slideMasterTextStyles"];
    var dirLoc: any = "";
    ...
    if (slideMasterTextStyles !== undefined && dirLoc !== "") {
        var dirVal: any = PPTXUtils.getTextByPathList(slideMasterTextStyles[dirLoc], ["a:lvl1pPr", "attrs", "rtl"]);
        if (dirVal == "1") {
            return "content-rtl";
        }
    } 
    // else {
    //     if (type == "textBox") {
    //         var dirVal = PPTXUtils.getTextByPathList(warpObj, ["defaultTextStyle", "a:lvl1pPr", "attrs", "rtl"]);
    //         if (dirVal == "1") {
    //             return "content-rtl";
    //         }
    //     }
 ...
 */
    static getVerticalAlign(node, slideLayoutSpNode, slideMasterSpNode, type) {
    //X, <a:bodyPr anchor="ctr">, <a:bodyPr anchor="b">
    var anchor: any = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:bodyPr", "attrs", "anchor"]);
    ...
    if (anchor === undefined) {
        ...
        anchor = PPTXUtils.getTextByPathList(slideLayoutSpNode, ["p:txBody", "a:bodyPr", "attrs", "anchor"]);
        if (anchor === undefined) {
            anchor = PPTXUtils.getTextByPathList(slideMasterSpNode, ["p:txBody", "a:bodyPr", "attrs", "anchor"]);
            if (anchor === undefined) {
                //"If this attribute is omitted, then a value of t, or top is implied."
                anchor = "t";//PPTXUtils.getTextByPathList(slideMasterSpNode, ["p:txBody", "a:bodyPr", "attrs", "anchor"]);
            }
        }
    }
 ...
 */
    ...
    if (algn === undefined) {
        ...
        if (algn === undefined && idx !== undefined ) {
            //slidelayout
            algn = PPTXUtils.getTextByPathList(warpObj["slideLayoutTables"]["idxTable"][idx], ["p:txBody", "a:lstStyle", lvlStr, "attrs", "algn"]);
            if (algn === undefined) {
                algn = PPTXUtils.getTextByPathList(warpObj["slideLayoutTables"]["idxTable"][idx], ["p:txBody", "a:p", "a:pPr", "attrs", "algn"]);
                if (algn === undefined) {
                    algn = PPTXUtils.getTextByPathList(warpObj["slideLayoutTables"]["idxTable"][idx], ["p:txBody", "a:p", (lvlIdx - 1), "a:pPr", "attrs", "algn"]);
                }
            }
        }
        if (algn === undefined) {
            if (type !== undefined) {
                //slidelayout
                algn = PPTXUtils.getTextByPathList(warpObj, ["slideLayoutTables", "typeTable", type, "p:txBody", "a:lstStyle", lvlStr, "attrs", "algn"]);

                if (algn === undefined) {
                    //masterlayout
                    if (type == "title" || type == "ctrTitle") {
                        algn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:titleStyle", lvlStr, "attrs", "algn"]);
                    } else if (type == "body" || type == "obj" || type == "subTitle") {
                        algn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:bodyStyle", lvlStr, "attrs", "algn"]);
                    } else if (type == "shape" || type == "diagram") {
                        algn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:otherStyle", lvlStr, "attrs", "algn"]);
                    } else if (type == "textBox") {
                        algn = PPTXUtils.getTextByPathList(warpObj, ["defaultTextStyle", lvlStr, "attrs", "algn"]);
                    } else {
                        algn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTables", "typeTable", type, "p:txBody", "a:lstStyle", lvlStr, "attrs", "algn"]);
                    }
                }
            } else {
                algn = PPTXUtils.getTextByPathList(warpObj, ["slideMasterTextStyles", "p:bodyStyle", lvlStr, "attrs", "algn"]);
 ...
 */
    static getPregraphMargn(pNode, idx, type, isBullate, warpObj){
    if (!isBullate){
        return ["",0];
    }
    var marLStr: any = "", marRStr = "" , maginVal = 0;
    var pPrNode: any = pNode["a:pPr"];
    var layoutMasterNode: any = PPTXLayoutUtils.getLayoutAndMasterNode(pNode, idx, type, warpObj);
    var pPrNodeLaout: any = layoutMasterNode.nodeLaout;
    var pPrNodeMaster: any = layoutMasterNode.nodeMaster;
    ...
    if ((indentNode !== undefined || marLNode !== undefined)) {
        ...
        if (isBullate) {
            ...
            marLStr += maginVal + "px;";  // Note: should subtract bullet numeric length/size for precise alignment
        } else {
    ...
    if (marRNode !== undefined && isBullate) {
        var marginRight: any = parseInt(marRNode) * this.slideFactor;
        if (isRTL) {// && alignNode == "r") {
 ...
 */
    static getVerticalMargins(pNode, textBodyNode, type, idx, warpObj) {
    //margin-top ; 
    ...
    if (PPTXUtils.getTextByPathList(pNode, ["a:r"]) !== undefined) {
        var fontSizeStr: any = PPTXTextStyleUtils.getFontSize(pNode["a:r"], textBodyNode,undefined, lvl, type, warpObj);
        if (fontSizeStr != "inherit") {
            fontSize = parseInt(fontSizeStr, 10); //pt
        }
    }
    //var spcBef = "";
    ...
    //     spcBefNode = PPTXUtils.getTextByPathList(pNode, ["a:pPr", "a:spcBef", "a:spcPct","attrs","val"]);
    //     if(spcBefNode !== undefined){
    ...
    // else{
    //    //i did not found case with percentage 
    //     spcAftNode = PPTXUtils.getTextByPathList(pNode, ["a:pPr", "a:spcAft", "a:spcPct","attrs","val"]);
    //     if(spcAftNode !== undefined){
    ...
    // }
    var isInLayoutOrMaster: any = true;
    if(type == "shape" || type == "textBox"){
    ...
    if (isInLayoutOrMaster && (spcBefNode === undefined || spcAftNode === undefined || lnSpcNode === undefined)) {
        //check in layout
        if (idx !== undefined) {
            var laypPrNode: any = PPTXUtils.getTextByPathList(warpObj, ["slideLayoutTables", "idxTable", idx, "p:txBody", "a:p", (lvl - 1), "a:pPr"]);

            if (spcBefNode === undefined) {
                spcBefNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:spcBef", "a:spcPts", "attrs", "val"]);
                // if(spcBefNode !== undefined){
                //     spcBef = "margin-top:" + parseInt(spcBefNode)/100 + "pt;"
                // } 
                // else{
                //    //i did not found case with percentage 
                //     spcBefNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:spcBef", "a:spcPct","attrs","val"]);
                //     if(spcBefNode !== undefined){
                //         spcBef = "margin-top:" + parseInt(spcBefNode)/100 + "%;"
                //     }
                // }
            ...
            if (spcAftNode === undefined) {
                spcAftNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:spcAft", "a:spcPts", "attrs", "val"]);
                // if(spcAftNode !== undefined){
                //     spcAft = "margin-bottom:" + parseInt(spcAftNode)/100 + "pt;"
                // }
                // else{
                //    //i did not found case with percentage 
                //     spcAftNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:spcAft", "a:spcPct","attrs","val"]);
                //     if(spcAftNode !== undefined){
                //         spcBef = "margin-bottom:" + parseInt(spcAftNode)/100 + "%;"
                //     }
                // }
            ...
            if (lnSpcNode === undefined) {
                lnSpcNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:lnSpc", "a:spcPct", "attrs", "val"]);
                if (lnSpcNode === undefined) {
                    lnSpcNode = PPTXUtils.getTextByPathList(laypPrNode, ["a:pPr", "a:lnSpc", "a:spcPts", "attrs", "val"]);
                    if (lnSpcNode !== undefined) {
                        lnSpcNodeType = "Pts";
                    }
                }
    ...
    if (isInLayoutOrMaster && (spcBefNode === undefined || spcAftNode === undefined || lnSpcNode === undefined)) {
        ...
        //     lvl = "a:lvl1pPr";
        // }
        var inLvlNode: any = PPTXUtils.getTextByPathList(slideMasterTextStyles, [dirLoc, lvlStr]);
        if (inLvlNode !== undefined) {
            if (spcBefNode === undefined) {
                ...
                // else{
                //    //i did not found case with percentage 
                //     spcBefNode = PPTXUtils.getTextByPathList(inLvlNode, ["a:spcBef", "a:spcPct","attrs","val"]);
                //     if(spcBefNode !== undefined){
                //         spcBef = "margin-top:" + parseInt(spcBefNode)/100 + "%;"
                //     }
                // }
            ...
            if (spcAftNode === undefined) {
                spcAftNode = PPTXUtils.getTextByPathList(inLvlNode, ["a:spcAft", "a:spcPts", "attrs", "val"]);
                // if(spcAftNode !== undefined){
                //     spcAft = "margin-bottom:" + parseInt(spcAftNode)/100 + "pt;"
                // }
                // else{
                //    //i did not found case with percentage 
                //     spcAftNode = PPTXUtils.getTextByPathList(inLvlNode, ["a:spcAft", "a:spcPct","attrs","val"]);
                //     if(spcAftNode !== undefined){
                //         spcBef = "margin-bottom:" + parseInt(spcAftNode)/100 + "%;"
                //     }
                // }
            ...
            if (lnSpcNode === undefined) {
                lnSpcNode = PPTXUtils.getTextByPathList(inLvlNode, ["a:lnSpc", "a:spcPct", "attrs", "val"]);
                if (lnSpcNode === undefined) {
                    lnSpcNode = PPTXUtils.getTextByPathList(inLvlNode, ["a:pPr", "a:lnSpc", "a:spcPts", "attrs", "val"]);
                    if (lnSpcNode !== undefined) {
                        lnSpcNodeType = "Pts";
                    }
                }
    ...
    var spcBefor: any = 0, spcAfter = 0, spcLines = 0;
    var marginTopBottomStr: any = "";
    if (spcBefNode !== undefined) {
    ...
    if (lnSpcNode !== undefined && fontSize !== undefined) {
        if (lnSpcNodeType == "Pts") {
            ...
            var fct: any = parseInt(lnSpcNode) / 100000;
            spcLines = fontSize * (fct - 1) - fontSize;// fontSize *
            var pTop: any = (fct > 1) ? spcLines : 0;
            var pBottom: any = (fct > 1) ? fontSize : 0;
