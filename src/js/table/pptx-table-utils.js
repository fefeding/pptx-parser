...
(function() {
     ...
     */
    PPTXTableUtils.genTableInternal = function(node, warpObj, styleTable) {
        ...
        var tableHtml = "<table " + tblDir + " style='border-collapse: collapse;" +
            window.PPTXUtils.getPosition(xfrmNode, node, undefined, undefined) +
            window.PPTXUtils.getSize(xfrmNode, undefined, undefined) +
            " z-index: " + order + ";" +
            tbl_borders + ";" +
            tbl_bgcolor + "'>";
        ...
        for (var i = 0; i < trNodes.length; i++) {
            ...
            if (rowHeightParam !== undefined) {
                rowHeight = parseInt(rowHeightParam) * window.PPTXConstants.SLIDE_FACTOR;
     ...
     */
    PPTXTableUtils.getTableCellParams = function(tcNodes, getColsGrid, row_idx, col_idx, thisTblStyle, cellSource, warpObj, styleTable) {
        ...
        var text = window.PPTXTextElementUtils.genTextBody(tcNodes["a:txBody"], tcNodes, undefined, undefined, undefined, undefined, warpObj, total_col_width, styleTable);//tableStyles

        if (total_col_width != 0 /*&& row_idx == 0*/) {
            colWidth = parseInt(total_col_width) * window.PPTXConstants.SLIDE_FACTOR;
