...
static getFontType(node, type, warpObj, pFontStyle) {
    var typeface = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:latin", "attrs", "typeface"]);
    ...
    if (typeface === undefined) {
        ...
        var fontSchemeNode = PPTXUtils.getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:fontScheme"]);
        if (fontIdx == "") {
...
static getTextHorizontalAlign(node, pNode, type, warpObj) {
    var getAlgn = PPTXUtils.getTextByPathList(node, ["a:pPr", "attrs", "algn"]);
...
static getFontSize(node, textBodyNode, pFontStyle, lvl, type, warpObj) {
    ...
    if (textBodyNode !== undefined){
        var spAutoFitNode = PPTXUtils.getTextByPathList(textBodyNode, ["a:bodyPr", "a:spAutoFit"]);
    ...
    if (!isNaN(fontSize)){
        var normAutofit = PPTXUtils.getTextByPathList(textBodyNode, ["a:bodyPr", "a:normAutofit", "attrs", "fontScale"]);
 ...
 */
    ...
    var rPrNode = PPTXUtils.getTextByPathList(node, ["a:rPr"]);
    ...
    if (rPrNode !== undefined) {
        ...
        if (filTyp == "SOLID_FILL") {
            var solidFillNode = rPrNode["a:solidFill"];// PPTXUtils.getTextByPathList(node, ["a:rPr", "a:solidFill"]);
            ...
            var pattFill = rPrNode["a:pattFill"];// PPTXUtils.getTextByPathList(node, ["a:rPr", "a:pattFill"]);
            color = PPTXColorUtils.getPatternFill(pattFill, warpObj);
            colorType = "pattern";
        } else if (filTyp == "PIC_FILL") {
            var picFillResult = PPTXColorUtils.getPicFill("slideBg", rPrNode["a:blipFill"], warpObj);
...
shpFill = lstStyledefRPr["a:gradFill"];
    ...
    if (color === undefined) {
        var sPstyle = PPTXUtils.getTextByPathList(pNode, ["p:style", "a:fontRef"]);
...
highlightNode = PPTXUtils.getTextByPathList(pPrNodeMaster, ["a:defRPr", "a:highlight"]);
    ...
    if (txtBrdrNode !== undefined && txtBrdrNode["a:noFill"] === undefined) {
        var txBrd = PPTXStyleManager.getBorder(node, pNode, false, "text", warpObj);
    ...
    var txtGlowNode = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:effectLst", "a:glow"]);
    var oGlowStr = "";
    ...
    var txtShadow = PPTXUtils.getTextByPathList(node, ["a:rPr", "a:effectLst", "a:outerShdw"]);
    var oShadowStr = "";
    if (txtShadow !== undefined) {
        ...
        var shadowClr = PPTXColorUtils.getSolidFill(txtShadow, undefined, undefined, warpObj);
 ...
 */
    ...
    var defRtl = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:lstStyle", "a:defPPr", "attrs", "rtl"]);
    if (defRtl !== undefined) {
        if (defRtl == "1"){
            return "content-rtl";
        } else if (defRtl == "0") {
    ...
    //     }
    // }
    var rtlCol = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:bodyPr", "attrs", "rtlCol"]);
    if (rtlCol !== undefined) {
        if (rtlCol == "1") {
            return "content-rtl";
        } else if (rtlCol == "0") {
    ...
    if (slideMasterTextStyles !== undefined && dirLoc !== "") {
        var dirVal = PPTXUtils.getTextByPathList(slideMasterTextStyles[dirLoc], ["a:lvl1pPr", "attrs", "rtl"]);
        if (dirVal == "1") {
            return "content-rtl";
        }
    } 
 ...
 */
    ...
    var anchor = PPTXUtils.getTextByPathList(node, ["p:txBody", "a:bodyPr", "attrs", "anchor"]);
 ...
 */
    ...
    //align
    var alignNode = PPTXUtils.getTextByPathList(pPrNode, ["attrs", "algn"]); //"l" | "ctr" | "r" | "just" | "justLow" | "dist" | "thaiDist
    ...
    //indent?
    var indentNode = PPTXUtils.getTextByPathList(pPrNode, ["attrs", "indent"]);
    ...
    //marR?
    var marRNode = PPTXUtils.getTextByPathList(pPrNode, ["attrs", "marR"]);
 ...
 */
    ...
    if (PPTXUtils.getTextByPathList(pNode, ["a:r"]) !== undefined) {
        var fontSizeStr = PPTXTextStyleUtils.getFontSize(pNode["a:r"], textBodyNode,undefined, lvl, type, warpObj);
    ...
    if (isInLayoutOrMaster && (spcBefNode === undefined || spcAftNode === undefined || lnSpcNode === undefined)) {
        //check in layout
        if (idx !== undefined) {
            var laypPrNode = PPTXUtils.getTextByPathList(warpObj, ["slideLayoutTables", "idxTable", idx, "p:txBody", "a:p", (lvl - 1), "a:pPr"]);
