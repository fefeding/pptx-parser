...
class PPTXBulletUtils {
    ...
    var rNode = PPTXUtils.getTextByPathList(node, ["a:r"]);
    if (rNode !== undefined && rNode.constructor === Array) {
        rNode = rNode[0]; //bullet only to first "a:r"
    }
    var lvl = parseInt(PPTXUtils.getTextByPathList(node["a:pPr"], ["attrs", "lvl"])) + 1;
    if (isNaN(lvl)) {
        lvl = 1;
    }
    ...
    var BullNONE = PPTXUtils.getTextByPathList(pPrNode, ["a:buNone"]);
    ...
    var buChar = PPTXUtils.getTextByPathList(pPrNode, ["a:buChar", "attrs", "char"]);
    var buNum = PPTXUtils.getTextByPathList(pPrNode, ["a:buAutoNum", "attrs", "type"]);
    var buPic = PPTXUtils.getTextByPathList(pPrNode, ["a:buBlip"]);
    if (buChar !== undefined) {
        buType = "TYPE_BULLET";
    }
    if (buNum !== undefined) {
    ...
    var buFontSize = PPTXUtils.getTextByPathList(pPrNode, ["a:buSzPts", "attrs", "val"]);
    ...
    var buClrNode = PPTXUtils.getTextByPathList(pPrNode, ["a:buClr"]);
    ...
    if (buType == "TYPE_BULLET") {
        var typefaceNode = PPTXUtils.getTextByPathList(pPrNode, ["a:buFont", "attrs", "typeface"]);
        var typeface = "";
        ...
        var buPicId = PPTXUtils.getTextByPathList(buPic, ["a:blip", "attrs", "r:embed"]);
        var svgPicPath = "";
        var buImg;
