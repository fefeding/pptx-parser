/**
 * PPTX Shape Container Utils
 * 处理形状的 SVG 容器、填充、边框等通用逻辑
 */
...
(function() {
    ...
    function getShapeTransformParams(node) {
        ...
        result.rotate = window.PPTXColorUtils.angleToDegrees(window.PPTXUtils.getTextByPathList(slideXfrmNode, ["attrs", "rot"]));
        ...
        if (txtXframeNode !== undefined) {
            ...
            if (txtXframeRot !== undefined) {
                result.txtRotate = window.PPTXColorUtils.angleToDegrees(txtXframeRot) + 90;
            }
        } else {
    ...
    // 导出函数到全局对象
    if (!window.PPTXShapeContainer) {
        window.PPTXShapeContainer = {};
    }
    ...
    window.PPTXShapeContainer.getDefsContent = getDefsContent;
