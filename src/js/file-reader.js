...
const workerScript = "self.addEventListener('message', function(e) { var data=e.data; try { var reader = new FileReaderSync; postMessage({ result: reader[data.readAs](data.file), extra: data.extra, file: data.file})} catch(e){ postMessage({ result:'error', extra:data.extra, file:data.file}); } }, false);";
const syncDetectionScript = "onmessage = function(e) { postMessage(!!FileReaderSync); };";
const fileReaderEvents = ['loadstart', 'progress', 'load', 'abort', 'error', 'loadend'];
let sync = false;

export const FileReaderJS = {
    enabled: false,
...
if (!FileReader) {
    ...
    throw 'FileReader is not supported';
}
