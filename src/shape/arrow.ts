    ...
    PPTXArrowShapes.genRightArrow = function(w, h, node, slideFactor) {
        ...
        let sAdj1: any, sAdj1_val = 0.25;
        let sAdj2: any, sAdj2_val = 0.5;
        ...
        if (shapAdjst_ary !== undefined) {
            for (let i = 0; i < shapAdjst_ary.length; i++) {
                ...
                if (sAdj_name == "adj1") {
                    ...
                    const sAdj2_val: any = 1 - ((sAdj2_val2) / max_sAdj2_const);
                }
            }
        }
...
const sAdj_name: any = PPTXUtils.getTextByPathList(shapAdjst_ary[i], ["attrs", "name"]);
                if (sAdj_name == "adj1") {
                    const sAdj1: any = PPTXUtils.getTextByPathList(shapAdjst_ary[i], ["attrs", "fmla"]);
                    const sAdj1_val: any = parseInt(sAdj1.substr(4)) / 200000;
                } else if (sAdj_name == "adj2") {
                    const sAdj2: any = PPTXUtils.getTextByPathList(shapAdjst_ary[i], ["attrs", "fmla"]);
const sAdj2_val2: any = parseInt(sAdj2.substr(4)) / 100000;
                    const sAdj2_val: any = (sAdj2_val2) / max_sAdj2_const;
                }
            }
        }
